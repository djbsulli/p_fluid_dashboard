...
PALETTE = {
    ...
    'accent':    '#8B1A1A',   # deep red — primary data colour (matches Reds palette)
    'accent_soft':'#C9302C',  # softer red — secondary highlights
...
TOUCHES_GDRIVE_ID = "1CWAHjO_Cxvq_e-aSFXp9XFFarF0LVNYC"
...
def draw_binned_touch_map(touch_df, ax, show_labels=True):
    ...
    # Reds palette — matches the dissertation codebook
    for zone_num in range(1, 10):
        ...
        # Auto-switch text colour: dark cells use white, light cells use black
        text_color = 'white' if pct >= 22 else PALETTE['ink']
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with tab_match:
        ...
        with col_mswarm:
            ...
            # Plot all other player-match combos as background using a strip plot
            # (much more render-efficient than swarmplot at thousands of points)
            mask_other = ~(
            ...
            if len(background) > 0:
                sns.stripplot(
                    ...
                    color=PALETTE['ink_muted'], size=3, alpha=0.4,
                    jitter=0.35, ax=ax
                )
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with tab_team_season:
        ...
        # Zonal Fluidity Averages — vertical bar chart with directional note
        st.markdown('<div class="section-header">Zonal Fluidity Averages</div>',
                    unsafe_allow_html=True)

        col_zone_plot, col_zone_note = st.columns([3, 1])
        ...
        with col_zone_plot:
            ...
            ax.bar(x_pos, zone_values, width=0.55,
        ...
        with col_zone_note:
            st.markdown(f"""
            <div style='background-color:{PALETTE['panel']}; border:1px solid {PALETTE['border']};
        ...
        # All-Teams Season Distribution — swarm plot mirroring the player season view
        st.markdown('<div class="section-header">All Teams Season Distribution</div>',
                    unsafe_allow_html=True)

        col_swarm_plot, col_swarm_note = st.columns([3, 1])

        with col_swarm_plot:
            all_teams_data = team_season_stats.copy()

            fig, ax = plt.subplots(figsize=(8, 5))
            fig.patch.set_facecolor(PALETTE['bg'])
            ax.set_facecolor(PALETTE['bg'])

            other_teams = all_teams_data[all_teams_data['team'] != selected_team]
            if len(other_teams) > 0:
                sns.swarmplot(
                    data=other_teams, x=np.zeros(len(other_teams)), y='season_avg',
                    color=PALETTE['ink_muted'], size=6, alpha=0.55, ax=ax
                )

            sel_team_data = all_teams_data[all_teams_data['team'] == selected_team]
            if len(sel_team_data) > 0:
                ax.scatter(
                    [0], sel_team_data['season_avg'].values[0],
                    color=PALETTE['accent'], s=140, zorder=10,
                    edgecolors=PALETTE['ink'], linewidths=1.5
                )
            ...
            ax.axhline(y=0, color=PALETTE['ink'], linestyle='-',
                       linewidth=1.5, alpha=0.7)
            ax.set_xlabel('All Teams', fontsize=11, fontweight='bold')
            ax.set_ylabel('Season Fluidity (avg_z)', fontsize=11, fontweight='bold')
            ax.tick_params(axis='x', bottom=False, labelbottom=False)
            ax.grid(axis='y', alpha=0.3, color=PALETTE['grid'])
            plt.tight_layout()
        ...
        with col_swarm_note:
            ...
            <div style='background-color:{PALETTE['panel']}; border:1px solid {PALETTE['border']};
                        ...
                        font-size:0.85rem; color:{PALETTE['ink_soft']}; line-height:1.5;'>
            <b style='color:{PALETTE['ink']};'>Note</b><br>
            Each grey point represents a team's season-average fluidity (avg_z) across all four leagues.
            The <b style='color:{PALETTE['accent']};'>red point</b> highlights
            <b style='color:{PALETTE['ink']};'>{selected_team}</b>, allowing comparison of the team's
            seasonal fluidity against the full population of teams in the dataset.
            </div>
            """, unsafe_allow_html=True)
        ...
        # Connecting line (background only — not in legend)
        ax.plot(team_matches_sorted['match_num'], team_matches_sorted['team_match_z'],
                color=PALETTE['ink_soft'], linewidth=1.2, zorder=2, alpha=0.6)

        # Coloured markers (fluid vs positional) — the only legend items
    ...
    with tab_team_match:
        ...
        # Comparison to Season — line plot showing selected match against season trajectory
        ...
        # Neutral line + markers for all matches — no fluid/positional colour coding here
        ax.plot(team_matches_sorted['match_num'], team_matches_sorted['team_match_z'],
                color=PALETTE['ink_soft'], linewidth=1.2, zorder=2, alpha=0.6)
        ax.scatter(team_matches_sorted['match_num'], team_matches_sorted['team_match_z'],
                   color=PALETTE['ink_soft'], s=40, zorder=3,
                   edgecolors=PALETTE['ink'], linewidths=0.5, alpha=0.7)
        ...
        # Prominent z=0 line — not included in legend
        ...
        col_mzone_plot, col_mzone_note = st.columns([3, 1])
        ...
        # All Team-Matches Distribution — strip plot mirroring the player match view
        st.markdown('<div class="section-header">All Team-Matches Distribution</div>',
                    unsafe_allow_html=True)

        col_tm_strip_plot, col_tm_strip_note = st.columns([3, 1])

        with col_tm_strip_plot:
            all_team_matches = team_match_stats.copy()

            fig, ax = plt.subplots(figsize=(8, 5))
            fig.patch.set_facecolor(PALETTE['bg'])
            ax.set_facecolor(PALETTE['bg'])

            # Plot all other team-match combinations as background using a strip plot
            # (more render-efficient than swarmplot at hundreds/thousands of points)
            mask_other_tm = ~(
                (all_team_matches['team'] == selected_team) &
                (all_team_matches['match_id'] == selected_match_id)
            )
            background_tm = all_team_matches[mask_other_tm]

            if len(background_tm) > 0:
                sns.stripplot(
                    data=background_tm, x=np.zeros(len(background_tm)), y='team_match_z',
                    color=PALETTE['ink_muted'], size=3, alpha=0.4,
                    jitter=0.35, ax=ax
                )

            # Highlight the selected team-match combination
            highlight_tm = all_team_matches[
                (all_team_matches['team'] == selected_team) &
                (all_team_matches['match_id'] == selected_match_id)
            ]
            if len(highlight_tm) > 0:
                ax.scatter(
                    [0], highlight_tm['team_match_z'].values[0],
                    color=PALETTE['accent'], s=160, zorder=10,
                    edgecolors=PALETTE['ink'], linewidths=1.5
                )
            ...
            ax.axhline(y=0, color=PALETTE['ink'], linestyle='-',
                       linewidth=1.5, alpha=0.7)
            ax.set_xlabel('All Team-Matches', fontsize=11, fontweight='bold')
            ax.set_ylabel('Match Fluidity (z-score)', fontsize=11, fontweight='bold')
            ax.tick_params(axis='x', bottom=False, labelbottom=False)
            ax.grid(axis='y', alpha=0.3, color=PALETTE['grid'])
            plt.tight_layout()
        ...
        with col_tm_strip_note:
            opp_name = match_row.get('opponent', 'opponent')
            st.markdown(f"""
            <div style='background-color:{PALETTE['panel']}; border:1px solid {PALETTE['border']};
                        border-radius:4px; padding:14px 16px; margin-top:8px;
                        font-size:0.85rem; color:{PALETTE['ink_soft']}; line-height:1.5;'>
            <b style='color:{PALETTE['ink']};'>Note</b><br>
            Each grey point represents a single team-match fluidity z-score across the full dataset.
            The <b style='color:{PALETTE['accent']};'>red point</b> highlights
            <b style='color:{PALETTE['ink']};'>{selected_team} vs {opp_name}</b>, allowing comparison
            of this match's fluidity against the full population of team-match performances.
            </div>
            """, unsafe_allow_html=True)
