...
if page == "Home":
    ...
    with col1:
        ...
        This has been characterised by the contrast between tactical systems championing positional fluidity,
        characterised by players having creative freedom to move around the pitch, and more organised, structured football.</p>
        <p>Positional fluidity, as measured in this project, refers to the degree to which a player operates
    ...
    with col2:
        ...
        twenty-first century, positional play (juego de posición), an attacking approach centred upon
        ...
        positionally fluid and expressive approaches, often grouped under the term relational play.</p>
    ...
    with tab1:
        ...
        <p>Open-play, outfield ball touch data was extracted for this project, removing any ball touches by
        goalkeepers and from set-pieces. This ensured that all data pertained to open-play attacking possession.
        A minimum of 15 open-play touches in a defined position (see positions below) per match was required for
        inclusion. Data was collected from the highest leagues in English, Spanish, Italian and German football,
        from the 2015-16 league season. These are the most recent full league seasons of data openly accessible.</p>
        <h4>Step 2 — Zone Assignment</h4>
        <p>The pitch was divided into a 9-zone grid (3 columns × 3 rows), splitting the pitch into the defensive,
        middle and attacking thirds horizontally, and left, central and right channels vertically. The grid is
        visualised below. StatsBomb coordinates are normalised to an attacking direction of left to right throughout
        a match, as represented in the touch maps throughout this dashboard.</p>
        <h4>Step 3 — Fluidity Percentage</h4>
        <p>For each qualifying player-position-match combination, the fluidity score was calculated as the
        percentage of touches falling outside the player's most frequently occupied zone:
        (touches outside modal zone / total touches) × 100. Higher scores indicate more fluid,
        zone-crossing player movements.</p>
        <h4>Step 4 — Seasonal Aggregation</h4>
        <p>Players were required to meet the 15-touch threshold in at least 8 matches (per position) to be
        ...
        with col_grid:
            ...
            pitch_home = Pitch(pitch_type='statsbomb', line_color='#333333',
                               pitch_color='#f8f9fa')
            fig_grid, ax_grid = plt.subplots(figsize=(7, 4.5))
            fig_grid.patch.set_facecolor('#f8f9fa')
            ax_grid.set_facecolor('#f8f9fa')
            ...
            for y in y_bins_home[1:-1]:
                ax_grid.plot([0, 120], [y, y], color='#4a7fb5',
                             linewidth=2, zorder=4, linestyle='--')
            zone_num = 1
            for i in range(len(x_bins_home) - 1):
                for j in range(len(y_bins_home) - 1):
                    cx = (x_bins_home[i] + x_bins_home[i+1]) / 2
                    cy = (y_bins_home[j] + y_bins_home[j+1]) / 2
        ...
        with col_pos:
            ...
            <p style='font-weight:600; margin-bottom:8px;'>Positions included in analysis:</p>
            ...
            Positions are derived from StatsBomb event-level position data. A player qualifies under
            a position if they recorded 15+ touches in that position in a given match. A player may
            qualify under multiple positions across the season.
            </p>
            </div>
            """, unsafe_allow_html=True)
