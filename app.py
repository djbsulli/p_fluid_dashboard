...
def load_data():
    ...
    match_meta = pd.read_parquet(os.path.join(SCRIPT_DIR, "matches_metadata.parquet"))
    return avg_team, team_match, season_player, filtered_pl, match_meta
...
try:
    avg_team_fluidity, team_match_stats, season_player_stats, filtered_players, matches_metadata = load_data()
    data_loaded = True
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with tab_match:
        ...
        # Merge home/away info from matches_metadata
        meta_lookup = matches_metadata[['match_id', 'home_team', 'away_team']].copy()
        player_match_data = player_match_data.merge(meta_lookup, on='match_id', how='left')
        player_match_data['home_away'] = player_match_data.apply(
        ...
        player_match_data = player_match_data.sort_values('opponent').reset_index(drop=True)

        # Build dropdown options sorted by opponent with home/away
        match_options = [
            f"vs {row['opponent']} ({row['home_away']})"
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with tab_team_season:
        ...
        std_col = 'val-positive' if std_z >= 0 else 'val-negative'
        col1, col2, col3, col4 = st.columns(4)
        with col1:
            st.markdown(stat_box("Season Fluidity Z-Score", f"{team_row['season_z_score']:.3f}"), unsafe_allow_html=True)
        with col2:
            st.markdown(stat_box("Consistency Z-Score", f"{std_z:.3f}", colour=std_col), unsafe_allow_html=True)
        with col3:
    ...
    with tab_team_match:
        ...
        # Build team matches with opponent and home/away
        meta_lookup = matches_metadata[['match_id', 'home_team', 'away_team']].copy()
        team_matches_with_meta = team_matches.merge(meta_lookup, on='match_id', how='left')
        team_matches_with_meta['home_away'] = team_matches_with_meta.apply(
            lambda r: 'Home' if r['team'] == r['home_team'] else 'Away', axis=1
        )
        ...
        team_matches_sorted = team_matches_with_meta.sort_values('opponent').reset_index(drop=True)
        team_matches_sorted['match_num'] = range(1, len(team_matches_sorted) + 1)
