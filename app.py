...
<style>
    ...
    body, p, li, span, div, label { color: #2c3e50 !important; }
...
def load_data():
    import gdown
    avg_team = pd.read_csv(os.path.join(SCRIPT_DIR, "avg_team_scores.csv"))
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with tab_season:
        ...
        st.markdown(f'<div class="section-header">{selected_player_name}: Season Stats</div>',
                    unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with tab_team_match:
        ...
        with col_mzone:
            ...
            s_colours = ['#a8d5a2' if v >= 0 else '#f5b7b1' for v in seasonal_values]
        ...
        with col_minfo:
            def_diff = match_row.get('defensive_z', 0) - team_row['avg_defensive_score']
            mid_diff = match_row.get('midfield_z', 0) - team_row['avg_midfield_score']
            fwd_diff = match_row.get('forward_z', 0) - team_row['avg_forward_score']
            ...
            st.markdown(f"""
            <div class='info-card'>
                <div style='font-weight:700; font-size:1rem; color:#2c3e50; margin-bottom:12px;'>
                    vs {opponent_name}
                </div>
                <div style='margin-bottom:10px;'>
                    <span style='color:#555; font-size:0.85rem;'>Defence</span><br>
                    <span style='font-weight:700; font-size:1.05rem; color:{colour_diff(def_diff)};'>{fmt_diff(def_diff)} vs avg</span>
                </div>
                <div style='margin-bottom:10px;'>
                    <span style='color:#555; font-size:0.85rem;'>Midfield</span><br>
                    <span style='font-weight:700; font-size:1.05rem; color:{colour_diff(mid_diff)};'>{fmt_diff(mid_diff)} vs avg</span>
                </div>
                <div>
                    <span style='color:#555; font-size:0.85rem;'>Attack</span><br>
                    <span style='font-weight:700; font-size:1.05rem; color:{colour_diff(fwd_diff)};'>{fmt_diff(fwd_diff)} vs avg</span>
                </div>
            </div>
            """, unsafe_allow_html=True)
