...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with tab_team_season:
        ...
        season_consistency = max(
            (team_zscores > 0).mean() * 100,
            (team_zscores < 0).mean() * 100
        )
        ...
        with col4:
            st.markdown(stat_box("Consistency Score", f"{season_consistency:.0f}%"), unsafe_allow_html=True)
        with col5:
        ...
        # ── Consistency swarm plot ────────────────────────────────────────────
        consistency_pct = max(
            (team_matches_sorted['team_match_z'] > 0).mean() * 100,
            (team_matches_sorted['team_match_z'] < 0).mean() * 100
        )

        swarm_df = team_matches_sorted[['team_match_z']].copy()
        swarm_df['x'] = selected_team
        swarm_df['above_zero'] = swarm_df['team_match_z'] > 0

        col_line, col_swarm = st.columns([3, 1])
        ...
        with col_swarm:
            ...
            above = swarm_df[swarm_df['above_zero']]
            below = swarm_df[~swarm_df['above_zero']]

            if len(above) > 0:
                sns.swarmplot(data=above, x='x', y='team_match_z',
                              color='#4a7fb5', size=7, ax=ax, label='Above Average')
            if len(below) > 0:
                sns.swarmplot(data=below, x='x', y='team_match_z',
                              color='#e74c3c', size=7, ax=ax, label='Below Average')

            ax.axhline(0, color='black', linewidth=1.5, linestyle='--',
                       alpha=0.7, label='All Leagues Average')
            ax.set_ylim(-4.5, 4.5)
            ax.set_xlabel('')
            ax.set_ylabel('Match Fluidity Score', fontsize=11, fontweight='bold')
            ax.set_title(f'Consistency Score: {consistency_pct:.0f}%',
