...
VALID_TOUCHES_GDRIVE_ID = "1dh1SSWcmrFehTnygcC4-PLTG6YEJioUl"
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with tab_team_season:
        ...
        with col4:
            st.markdown(stat_box("Matches Included", int(team_row['matches'])), unsafe_allow_html=True)
    ...
    with tab_team_match:
        ...
        with col_xg:
            ...
            xg_col = 'val-positive' if pd.notna(xg_val) and xg_val >= xg_avg else 'val-negative'
            st.markdown(stat_box("Match xG", f"{xg_val:.2f}" if pd.notna(xg_val) else "N/A", colour=xg_col), unsafe_allow_html=True)
        with col_ft:
            ft_val = match_row.get('field_tilt')
            ft_avg = team_matches_sorted['field_tilt'].mean()
            ft_col = 'val-positive' if pd.notna(ft_val) and ft_val >= ft_avg else 'val-negative'
            st.markdown(stat_box("Match Field Tilt %", f"{ft_val:.1f}" if pd.notna(ft_val) else "N/A", colour=ft_col), unsafe_allow_html=True)
        ...
        with col_xg_line:
            ...
            ax.axhline(xg_season_avg, color='#e74c3c', linewidth=1.2, linestyle='--',
                       label=f'Season avg: {xg_season_avg:.2f}', zorder=1)
        ...
        with col_ft_line:
            ...
            ax.axhline(ft_season_avg, color='#e74c3c', linewidth=1.2, linestyle='--',
                       label=f'Season avg: {ft_season_avg:.1f}%', zorder=1)
