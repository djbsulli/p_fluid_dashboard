...
def stat_box(label, value):
    return (f"<div style='background:#ffffff; border:1px solid #dee2e6; border-radius:8px; "
            ...
            f"<div style='font-size:0.95rem; font-weight:600; color:#2c3e50;'>{value}</div>"
            f"</div>")
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with tab_match:
        ...
        with col2:
            st.markdown(stat_box("Difference from Season Average", f"{delta_pl:+.3f}"), unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with tab_team_match:
        ...
        with col2:
            st.markdown(stat_box("Difference from Season Average", f"{diff_from_avg:+.3f}"), unsafe_allow_html=True)
        with col3:
        ...
        with col_minfo:
            ...
            def zone_html(label, raw_val, avg_val):
                ...
                return (f"<div style='margin-bottom:10px;'>"
                        f"<span style='color:#555; font-size:0.85rem;'>{label}</span><br>"
                        f"<span style='font-weight:700; font-size:1.05rem; color:{colour_diff(diff)} !important;'>"
                        f"{fmt_diff(diff)}</span></div>")
