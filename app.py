...
<style>
    ...
    .stSelectbox div[data-baseweb="select"] * { color: #2c3e50 !important; }
    [data-baseweb="select"] { background-color: #ffffff !important; }
    [data-baseweb="select"] span { color: #2c3e50 !important; }
    [data-baseweb="popover"] * { color: #2c3e50 !important; background-color: #ffffff !important; }
    ...
    with tab_season:
        ...
        with col_map:
            ...
            if dominant_norm is not None:
                ...
                norm_note = f"Position: {dominant_norm} (dominant)" if len(all_norms) > 1 else f"Position: {dominant_norm}"
                ax.set_title(norm_note, fontsize=9, pad=6, color='#6c757d')
                plt.tight_layout()
                st.pyplot(fig)
                plt.close()
            else:
